    ...
    <>
      ...
      >
        <div className="container mx-auto px-6 grid grid-cols-3 items-center">
          ...
          <nav className="hidden md:flex space-x-8 text-[11px] uppercase tracking-[0.2em] font-medium">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.path}
                className={`group relative transition-colors hover:text-obsidian ${
                  pathname.startsWith(link.path) ? "text-obsidian" : "text-slate"
                }`}
              >
                {link.name}
                <motion.span 
                  className="absolute -bottom-1 left-0 w-0 h-[1px] bg-obsidian transition-all duration-300 group-hover:w-full"
          ...
          >
            <span className="hidden sm:inline-block text-xs md:text-sm font-serif tracking-[0.25em] text-obsidian uppercase">
          ...
          <div className="flex items-center space-x-4 md:space-x-6 justify-self-end">
            <button className="hidden md:block p-2 text-obsidian hover:opacity-60 transition">
              <Search className="w-4 h-4" />
            </button>
            <button
              onClick={() => setIsCartOpen(true)}
              className="relative p-2 text-obsidian hover:opacity-60 transition flex items-center"
            >
              ...
              <AnimatePresence>
                {cartCount > 0 && (
                  <motion.span 
                    ...
                    animate={{ scale: 1 }}
                    exit={{ scale: 0 }}
                    className="absolute -top-1 -right-1 bg-obsidian text-alabaster text-[8px] w-4 h-4 flex items-center justify-center rounded-full leading-none font-bold"
                  >
                    {cartCount}
                  </motion.span>
                )}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white z-50 md:hidden flex flex-col p-8"
          >
            <div className="flex justify-between items-center mb-12">
              <span className="font-serif tracking-widest text-sm text-slate">MENU</span>
